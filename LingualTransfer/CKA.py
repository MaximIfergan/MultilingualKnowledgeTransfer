...
if __name__ == '__main__':
    ...
    Y = np.random.randn(5, 64)
    X_p = X.copy()
    X_p[[4, 3, 0, 1]] = X[[0, 1, 4, 3]]

    print('Linear CKA, between X and Y: {}'.format(linear_CKA(X, Y)))
    print('Linear CKA, between X and X: {}'.format(linear_CKA(X, X)))
    print('Linear CKA, between X and Xp: {}'.format(linear_CKA(X, X_p)))
    print('Linear CKA, between Xp and Y: {}'.format(linear_CKA(X_p, Y)))
